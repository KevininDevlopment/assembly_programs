...
extern "C" int CalcMatrixRowColSums_(int* row_sums, int* col_sums,
	const int* x, int nrows, int ncols);
...
void PrintResult(const char* msg, const int* row_sums, const int* col_sums,
	const int* x, int nrows, int ncols)
{
	const int w = 6;
	const char nl = '\n';
...
int CalcMatrixRowColSumsCpp(int* row_sums, int* col_sums, const int* x,
	int nrows, int ncols)
{
	int rc = 0;
...
int main()
{
	...
	int rc1 = CalcMatrixRowColSumsCpp(row_sums1, col_sums1, (int*)x,
		nrows, ncols);
	int rc2 = CalcMatrixRowColSums_(row_sums2, col_sums2, (int*)x, nrows,
		ncols);
